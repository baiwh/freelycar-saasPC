  ...
  <div>
    ...
    <el-tabs v-model="item">
      ...
      <el-tab-pane label="项目管理" name="first">
        ...
        >
          ...
          <el-table-column align="center" label="序号" type="index">
            <template slot-scope="scope">
              <span>{{scope.$index+(pageData2.currentPage - 1) * pageData2.pageSize + 1}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" prop="saleStatus" label="智能柜服务">
            <template slot-scope="scope">
              ...
              <el-button
                size="mini"
                :type="!scope.row.saleStatus ? 'info': 'success'"
                v-popover="scope.row.name"
              >{{!scope.row.saleStatus ? '开通':'取消'}}</el-button>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" label="排序" width="150">
            <template slot-scope="scope">
              ...
              >上移</el-button>
              <el-button
                :disabled="scope.$index===(tableData2.length-1)"
                size="mini"
                type="primary"
        ...
        <!--分页器-->
        <el-row class="show">
          <pagingDevice :pageData.sync="pageData2" @changePage="getTableData2"></pagingDevice>
        </el-row>
      </el-tab-pane>
      ...
      <el-tab-pane label="项目类别" name="second">
        ...
        <!--分页器-->
        <el-row class="show">
          <pagingDevice :pageData.sync="pageData1" @changePage="getTableData1"></pagingDevice>
        </el-row>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog
      ...
      :title="projectIsModify?'修改项目类别':'新增项目类别'"
    ...
    <el-dialog
      ...
      :title="projectIsModify2?'修改项目':'新增项目'"
...
export default {
  ...
  methods: {
      ...
      this.$refs["dialog2"].validate((valid) => {
        if (valid) {
          ...
          this.$post("/project/modify", {
            ...
            memberPrice:
              this.dialog2.memberPrice === "" ? null : this.dialog2.memberPrice,
    ...
    switchLocation(param) {
      this.$post("/project/switchLocation", param).then((res) => {
        this.$message({
          message: "移动成功",
          type: "success",
        });
